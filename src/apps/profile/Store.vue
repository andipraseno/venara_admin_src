          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3">
              <input
                ...
                id="maps_link"
                class="form-control"
                :class="{ 'is-invalid': errors.maps_link }"
                placeholder="Maps Link"
              />
              <label for="maps_link" class="form-label">Maps Link</label>
              <div class="invalid-feedback" v-if="errors.maps_link">{{ errors.maps_link[0] }}</div>
            </div>
...
export default {
  ...
  mounted() {
    ...
    $("#tblMain").on("click.store", ".btn-edit", this.dtEditClickHandler);
  ...
  beforeUnmount() {
    ...
    $("#tblMain").off("click.store", ".btn-edit");
  ...
  methods: {
    ...
    // ---------- DataTable init ----------
    initDataTable() {
      const url = `${this.baseUrl}/profile/store`;
    ...
    async openEditModal(id) {
      ...
      try {
        const url = `${this.baseUrl}/profile/store_show/${id}`;
        const res = await axios.get(url, this.tokenHeaders);
        const data = res.data || {};
    ...
    async submitForm() {
      ...
      const url = `${this.baseUrl}/profile/store_add`;
...
<style scoped src="@/apps/profile/store.css"></style>
