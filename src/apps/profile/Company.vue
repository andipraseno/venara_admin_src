...
export default {
  ...
  mounted() {
    ...
    $("#tblMain").on("click.Company", ".btn-edit", this.dtEditClickHandler);
  ...
  beforeUnmount() {
    ...
    $("#tblMain").off("click.Company", ".btn-edit");
  ...
  methods: {
    // ---------- load Holding / Company / Company ----------
    async loadHoldingList() {
      try {
        const url = `${this.baseUrl}/profile/holding_combo`;
        const res = await axios.get(url, this.tokenHeaders);
        this.holdingList = res.data || [];
      } catch (e) {
        alert("Gagal memuat daftar holding.");
      }
    },

    // ---------- DataTable init ----------
    initDataTable() {
      const url = `${this.baseUrl}/profile/company`;
    ...
    async openEditModal(id) {
      ...
      try {
        const url = `${this.baseUrl}/profile/company_show/${id}`;
        const res = await axios.get(url, this.tokenHeaders);
        const data = res.data || {};
    ...
    async submitForm() {
      ...
      const url = `${this.baseUrl}/profile/company_add`;
      const payload = {
...
<style scoped src="@/apps/profile/company.css"></style>
