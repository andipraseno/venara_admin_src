  ...
  methods: {
    ...
    initDataTable() {
      ...
      this.table = $("#tableMain").DataTable({
        ...
        pageLength: 25,
        ajax: {
          url: `${this.baseUrl}/profile/kode_transaksi`,
          type: "GET",
        ...
        columns: [
          { data: "nama" },
          { data: "kode", className: "text-center", width: "350px" },
          {
            data: "status",
    ...
    async openEditModal(id) {
      ...
      try {
        const res = await axios.get(
          `${this.baseUrl}/profile/kode_transaksi_show/${id}`,
          this.tokenHeaders
        );
        const data = res.data || {};
    ...
    async submitForm() {
      ...
      try {
        await axios.post(`${this.baseUrl}/profile/kode_transaksi_add`, payload, this.tokenHeaders);
...
<style scoped src="@/apps/profile/kodetransaksi.css"></style>
