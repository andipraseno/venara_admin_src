        ...
        <form @submit.prevent="submitEdit" class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editModalLabel">
              <i class="bi bi-credit-card-2-back me-2"></i>Start Payment
            </h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          ...
          <div class="modal-body">
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <div class="form-floating mb-3">
                  <textarea
                    v-model.trim="form.tipe_note"
                    id="editTipeNote"
                    class="form-control"
                    style="height: 150px"
                    :class="{ 'is-invalid': errors.tipe_note }"
                  ></textarea>
                  <label for="editTipeNote" class="form-label">Tipe Note</label>
                  <div class="invalid-feedback" v-if="errors.tipe_note">
                    {{ errors.tipe_note[0] }}
                  </div>
                </div>
                ...
                <div class="alert alert-info small d-flex align-items-start">
                  ...
                  <div>
                    Pastikan order benar sebelum menandai pembayaran sebagai <strong>lunas</strong>.
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    formatNumber(num) {
      ...
      return new Intl.NumberFormat("id-ID").format(Number(cleaned || 0));
    },
    ...
    async fetchJadwal() {
      const url = this.rootUrl + "/transaction/payment";
      const payload = {
    ...
    async openEditModal(id) {
      const url = this.rootUrl + `/transaction/payment/${id}`;
      const token = { headers: { Token: "Bearer " + localStorage.getItem("actasysToken") } };
      ...
      try {
        ...
        this.form.tipe_note = row.tipe_note ?? "";
    ...
    async submitEdit() {
      const url = this.rootUrl + `/transaction/payment_save`;
      const payload = {
        id: this.form.id,
        tipe_note: this.form.tipe_note,
      ...
      try {
        const ok = await Swal.fire({
          title: "Konfirmasi",
          text: `Tandai order #${this.form.no_order} sebagai LUNAS?`,
          icon: "question",
        ...
        this.resetForm();
        this.fetchJadwal();
        Swal.fire("Berhasil", "Payment berhasil.", "success");
      } catch (error) {
  ...
  beforeUnmount() {
    try {
      this.editModalInst && this.editModalInst.dispose();
    } catch (e) {
      // ignore
    }
  },
...
<style scoped src="@/apps/transaction/payment.css"></style>
