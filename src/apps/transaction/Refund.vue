  ...
  <div class="container-fluid" v-cloak>
    ...
    <div class="mb-4">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
          <h3 class="mb-1 fw-bold"><i class="bi bi-boxes"></i> Refund</h3>
            ...
            <tbody>
              <tr v-for="(item, i) in group.items" :key="`row-${gi}-${i}`" class="row-hover-soft">
                ...
                <td class="text-center">
                  <button class="btn btn-sm btn-danger" @click="openEditModal(item.id)">
        ...
        <form @submit.prevent="submitEdit" class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editModalLabel">
              <i class="bi bi-cash-refund me-2"></i>Start Refund
            </h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3">
              <input
                ...
                id="editTglOrder"
                class="form-control"
                :class="{ 'is-invalid': errors.tgl_order }"
                readonly
              />
              <label for="editTglOrder" class="form-label">Tgl. Order</label>
              <div class="invalid-feedback" v-if="errors.tgl_order">
                {{ errors.tgl_order[0] }}
              </div>
            </div>
            ...
            <div class="form-floating mb-3">
              <input
                ...
                id="editGrandTotal"
                class="form-control"
                :class="{ 'is-invalid': errors.grand_total }"
                readonly
              />
              <label for="editGrandTotal" class="form-label">Grand Total</label>
              <div class="invalid-feedback" v-if="errors.grand_total">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary" :disabled="saving">
              <span v-if="!saving"><i class="bi bi-save2 me-1"></i>Simpan</span>
              <span v-else>
                <span
...
export default {
  ...
  data() {
    const defaultTanggal = this.todayIndo();
    ...
    return {
      ...
      rootUrl: import.meta.env.VITE_API_APPS || "",
      editModalInst: null,
    };
  },
  ...
  methods: {
    ...
    async fetchJadwal() {
      const url = this.rootUrl + "/transaction/refund";
      const payload = {
    ...
    async openEditModal(id) {
      const url = `${this.rootUrl}/transaction/refund/${id}`;
      const token = { headers: { Token: "Bearer " + localStorage.getItem("actasysToken") } };
      ...
      try {
        ...
        this.form.tgl_order = this.toDmy(row.tgl_order) ?? "";
        this.form.nama = row.nama ?? "";
        ...
        const confirm = await Swal.fire({
          title: "Mulai proses refund?",
          text: `Order #${this.form.no_order} - ${this.form.nama}`,
          icon: "question",
    ...
    async submitEdit() {
      const url = `${this.rootUrl}/transaction/refund_save`;
      ...
      try {
        const ok = await Swal.fire({
          ...
          text: "Pastikan data bank & no. rekening sudah benar.",
          icon: "warning",
          showCancelButton: true,
        ...
        this.resetForm();
        this.fetchJadwal();
        Swal.fire("Berhasil", "Refund berhasil diproses.", "success");
      } catch (error) {
  ...
  beforeUnmount() {
    try {
      if (this.editModalInst?.dispose) {
        this.editModalInst.dispose();
      }
    } catch {
      // ignore
    }
...
<style scoped src="@/apps/transaction/refund.css"></style>
