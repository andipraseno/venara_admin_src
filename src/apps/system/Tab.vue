      ...
      <div class="modal-dialog">
        <form @submit.prevent="submitForm" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <!-- Software -->
            <div class="form-floating mb-3">
              <select class="form-select" v-model="form.software_id" id="formSoftware">
                <option value="">Pilih Software</option>
                <option v-for="s in softwareList" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
              </select>
              <label for="formSoftware" class="form-label">Software</label>
            </div>
      ...
      <div class="modal-dialog">
        <form @submit.prevent="doSearch" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3">
              <select class="form-select" v-model="filter.software_id" id="filterSoftware">
                <option value="">Pilih Software</option>
                <option v-for="s in softwareList" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
...
export default {
  ...
  mounted() {
    ...
    $("#tableTipeAbsen").on("click", ".btn-edit", this.dtEditClickHandler);
  ...
  beforeUnmount() {
    ...
    if (this.dtEditClickHandler) {
      $("#tableTipeAbsen").off("click", ".btn-edit", this.dtEditClickHandler);
    ...
    if ($.fn.dataTable && $.fn.dataTable.isDataTable("#tableTipeAbsen")) {
      try {
        $("#tableTipeAbsen").DataTable().destroy(true);
      } catch (e) {
        // ignore
      }
    }
  ...
  methods: {
    ...
    async loadSoftwareList() {
      try {
        ...
        alert("Gagal memuat daftar software. Silakan coba lagi.");
      }
    },
    ...
    async openEditModal(id) {
      ...
      try {
        ...
        this.form.urutan = typeof data.urutan !== "undefined" ? data.urutan : 1;
    ...
    async submitForm() {
      ...
      const payload = {
        ...
        software_id: this.form.software_id,
...
<style scoped src="@/apps/system/tab.css"></style>
