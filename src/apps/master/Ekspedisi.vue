        ...
        <form @submit.prevent="submitForm" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3">
              ...
              />
              <label>Telepon</label>
              <div class="invalid-feedback" v-if="errors.telepon">
                {{ errors.telepon[0] }}
              </div>
            </div>
  ...
  methods: {
    ...
    initDataTable() {
      ...
      this.table = $("#tableMain").DataTable({
        ...
        pageLength: 25,
        ajax: {
          url: `${this.baseUrl}/master/ekspedisi`,
          type: "GET",
    ...
    async openEditModal(id) {
      ...
      try {
        const res = await axios.get(
          `${this.baseUrl}/master/ekspedisi_show/${id}`,
          this.tokenHeaders
        );
        const data = res.data || {};
    ...
    async submitForm() {
      ...
      try {
        await axios.post(`${this.baseUrl}/master/ekspedisi_add`, payload, this.tokenHeaders);
...
<style scoped src="@/apps/master/Ekspedisi.css"></style>
