      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">
            <router-link :to="{ name: 'TransferStokTerima' }"> Transfer Stok Terima </router-link>
          </li>
        ...
        <div class="card shadow-sm sticky-top" style="top: 90px">
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" v-model="form.gudang_asal_id">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" v-model="form.gudang_tujuan_id">
                <option value="">Pilih Gudang</option>
                <option v-for="g in gudangList" :key="g.id" :value="g.id">
                  {{ g.nama }}
                </option>
                ...
                <tr v-for="(item, index) in items" :key="index">
                  ...
                  <td class="text-center">
                    <button
                      ...
                      @click="scanItem(item)"
                    >
                      <i class="bi bi-upc-scan"></i> Scan Confirm
                    </button>
                  </td>
                </tr>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      try {
        await this.$axios.post(
          `${this.baseUrl}/inventory/transfer_stok_add`,
          {
            ...this.form,
    ...
    async saveItem(item, index) {
      ...
      try {
        const res = await this.$axios.post(
          `${this.baseUrl}/inventory/transfer_stok_detail_add`,
          {
            id: item.id || null,
    ...
    async deleteItem(id, index) {
      ...
      await this.$axios.post(
        `${this.baseUrl}/inventory/transfer_stok_detail_delete`,
        { id },
        this.tokenHeaders
      );
    ...
    async initData() {
      ...
      const res = await axios.get(
        `${this.baseUrl}/inventory/transfer_stok_show/${this.form.id}`,
        this.tokenHeaders
      );
      ...
      if (!this.rfidTable) {
        this.$nextTick(async () => {
          ...
          $("#tableRfid").on("click", ".btn-broadcast", async (e) => {
            ...
            await axios.post(
              `${this.baseUrl}/inventory/transfer_stok_rfid_broadcast`,
              { rfid },
              this.tokenHeaders
            );
    ...
    async submitRfid() {
      ...
      try {
        // ================= SIMPAN KE API RFID =================
        await axios.post(
          `${this.baseUrl}/inventory/transfer_stok_rfid_add`,
          {
            header_id: this.form.id,
            barang_id: this.selectedScanItem.barang_id, // ⬅️ penting
            rfid: rfid,
          },
          this.tokenHeaders
        );
        ...
        this.rfidTable.row
          .add([
            ...
            rfid,
            `<button class="btn btn-sm btn-danger btn-broadcast">
    ...
    async loadRfidFromApi() {
      ...
      try {
        const res = await axios.post(
          `${this.baseUrl}/inventory/transfer_stok_rfid_show`,
          {
            header_id: this.form.id,
        ...
        data.forEach((x, i) => {
          ...
          this.rfidTable.row.add([
            i + 1,
            rfid,
            `<button class="btn btn-sm btn-primary btn-broadcast">
...
<style scoped src="@/apps/inventory/transferstokkirimdetail.css"></style>
