    ...
    <div class="mb-3 mt-3">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">Transfer Stok Terima</li>
        </ol>
      </nav>
    </div>
          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3 position-relative">
              <Cleave
                ...
                id="filterTglTransaksi"
                class="form-control pe-5"
                placeholder="dd/mm/yyyy"
              />
              <label for="filterTglTransaksi">Tgl. Transaksi</label>
            ...
            <div class="form-floating mb-3">
              <select
                class="form-select"
                v-model="form.gudang_asal_id"
                id="formGudangAsal"
                :class="{ 'is-invalid': errors.gudang_asal_id }"
              >
                <option value="">Pilih Gudang Asal</option>
                <option v-for="s in gudangasalList" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
              </select>
              <label for="formGudangAsal" class="form-label">Gudang Asal</label>
              <div class="invalid-feedback" v-if="errors.gudang_asal_id">
                {{ errors.gudang_asal_id[0] }}
              </div>
            </div>
            ...
            <div class="form-floating mb-3">
              <select
                class="form-select"
                v-model="form.gudang_tujuan_id"
                id="formGudangTujuan"
                :class="{ 'is-invalid': errors.gudang_tujuan_id }"
              >
                <option value="">Pilih Gudang Tujuan</option>
                <option v-for="s in gudangtujuanList" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
              </select>
              <label for="formGudangTujuan" class="form-label">Gudang Tujuan</label>
              <div class="invalid-feedback" v-if="errors.gudang_tujuan_id">
                {{ errors.gudang_tujuan_id[0] }}
              </div>
            </div>
          ...
          <div class="modal-body">
            <div class="form-floating mb-3">
              <input
                type="text"
                v-model="filter.no_transaksi"
                id="filterNo. Transaksi"
                class="form-control"
              />
              <label for="filterNo. Transaksi" class="form-label">No. Transaksi</label>
            </div>
...
export default {
  ...
  mounted() {
    ...
    this.loadGudangAsalList();
    this.loadGudangTujuanList();
    this.initDataTable();
    ...
    if (filterEl) {
      filterEl.addEventListener("shown.bs.modal", async () => {
        await this.$nextTick();
        const el = document.getElementById("filterNo. Transaksi");
        if (el) el.focus();
      });
    }
  },
  ...
  methods: {
    ...
    // load combo group
    async loadGudangAsalList() {
      try {
        const url = `${this.baseUrl}/master/gudang_combo`;
        const res = await axios.get(url, this.tokenHeaders);
        this.gudangasalList = res.data || [];
      } catch (e) {
        console.error("loadGudangAsalList", e);
        alert("Gagal memuat data. Silakan coba lagi.");
      }
    },

    async loadGudangTujuanList() {
      try {
        const url = `${this.baseUrl}/master/gudang_combo`;
        const res = await axios.get(url, this.tokenHeaders);
        this.gudangtujuanList = res.data || [];
      } catch (e) {
        console.error("loadGudangTujuanList", e);
        alert("Gagal memuat data. Silakan coba lagi.");
      }
    },
    ...
    // init datatable
    initDataTable() {
      const url = `${this.baseUrl}/inventory/transfer_stok`;
    ...
    async openEditModal(id) {
      ...
      const url = `${this.baseUrl}/inventory/transfer_stok_show/${id}`;
      try {
        ...
        this.form.gudang_asal_id = data.gudang_asal_id ?? "";
        this.form.gudang_tujuan_id = data.gudang_tujuan_id ?? "";
    ...
    async submitForm() {
      ...
      const url = `${this.baseUrl}/inventory/transfer_stok_add`;
      ...
      const payload = {
        ...
        gudang_tujuan_id: this.form.gudang_tujuan_id,
        memo: this.form.memo,
...
<style scoped src="@/apps/inventory/transferstokkirim.css"></style>
