    ...
    <div class="mb-3 mt-3">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 small">
          ...
          <li class="breadcrumb-item active" aria-current="page">Sub Brand Barang 3</li>
        </ol>
      </nav>
    </div>
        ...
        <form @submit.prevent="submitForm" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3">
              <select
                class="form-select"
                v-model="form.group1_id"
                id="group1Select"
                @change="onGroup1Change"
              >
                <option value="">Pilih Group 1</option>
                <option v-for="s in group1List" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
              </select>

              <label for="group1Select" class="form-label">Group 1</label>
            </div>

            <!-- Group2 -->
            <div class="form-floating mb-3">
              <select class="form-select" v-model="form.group2_id" id="group2Select">
                <option value="">Pilih Group 2</option>
                <option v-for="s in group2List" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
              </select>
              <label for="group2Select" class="form-label">Group 2</label>
            </div>
      ...
      <div class="modal-dialog">
        <form @submit.prevent="doSearch" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <!-- Group1 Filter -->
            <div class="form-floating mb-3">
              <select class="form-select" v-model="filter.group1_id" @change="onFilterGroup1Change">
                <option value="">Pilih Group 1</option>
                <option v-for="s in group1List" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
            ...
            <!-- Group2 Filter -->
            <div class="form-floating mb-3">
              <select class="form-select" v-model="filter.group2_id">
                <option value="">Pilih Group 2</option>
                <option v-for="s in group2List" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
...
export default {
  ...
  methods: {
    // ---------- load Group1 / Group2 / Sub Brand Barang 3 ----------
    async loadGroup1List() {
      try {
        const url = `${this.baseUrl}/lampiran/sub_brand_barang_1_combo`;
        const res = await axios.get(url, this.tokenHeaders);
        this.group1List = res.data || [];
      } catch (e) {
        console.error("loadGroup1List", e);
        alert("Gagal memuat daftar group1.");
      }
    },

    // load group2, bisa difilter per group1
    async loadGroup2List(group1Id = "") {
      try {
        const url = `${this.baseUrl}/lampiran/sub_brand_barang_2_combo/${group1Id}`;
        const res = await axios.get(url, this.tokenHeaders);
        this.group2List = res.data || [];
      } catch (e) {
        console.error("loadGroup2List", e);
      }
    },

    // ---------- DataTable init ----------
    initDataTable() {
      const url = `${this.baseUrl}/lampiran/sub_brand_barang_3`;
      ...
      this.table = $("#tblMain").DataTable({
        ...
        ajax: {
          ...
          data: (d) => {
            ...
            d.group1_id = this.filter.group1_id;
            d.group2_id = this.filter.group2_id;
    ...
    async openEditModal(id) {
      ...
      try {
        const url = `${this.baseUrl}/lampiran/sub_brand_barang_3_show/${id}`;
        const res = await axios.get(url, this.tokenHeaders);
        const data = res.data || {};

        this.form.id = id;
        this.form.nama = data.nama ?? "";
        this.form.group1_id = data.group1_id ?? "";
        this.form.group2_id = data.group2_id ?? "";
        this.form.status = typeof data.status !== "undefined" ? data.status : 1;

        // load cascading combos sesuai data
        await this.loadGroup1List();
        if (this.form.group1_id) {
          await this.loadGroup2List(this.form.group1_id);
        }
    ...
    async submitForm() {
      ...
      const url = `${this.baseUrl}/lampiran/sub_brand_barang_3_add`;
      const payload = {
        id: this.isEditMode ? this.form.id : "",
        nama: this.form.nama,
        group1_id: this.form.group1_id,
        group2_id: this.form.group2_id,
    ...
    // dipanggil saat dropdown group1 di form berubah
    onGroup1Change() {
      const group1Id = this.form.group1_id;
      this.form.group2_id = "";
      this.loadGroup2List(group1Id);
    },
  },
};
</script>

<style scoped src="@/apps/lampiran/subbrandbarang3.css"></style>
