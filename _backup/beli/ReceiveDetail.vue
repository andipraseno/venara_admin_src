      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">
            <router-link :to="{ name: 'Receive' }"> Receive </router-link>
          </li>
...
export default {
  ...
  methods: {
    ...
    async saveItem(item, index) {
      ...
      try {
        const res = await this.$axios.post(
          `${this.baseUrl}/beli/retur_detail_add`,
          {
            id: item.id || null,
    ...
    async deleteItem(id, index) {
      ...
      await this.$axios.post(
        `${this.baseUrl}/beli/retur_detail_delete`,
        { id },
        this.tokenHeaders
      );
    ...
    async initData() {
      ...
      const res = await axios.get(
        `${this.baseUrl}/beli/retur_show/${this.form.id}`,
        this.tokenHeaders
      );
      ...
      if (!this.rfidTable) {
        this.$nextTick(async () => {
          ...
          $("#tableRfid").on("click", ".btn-del", async (e) => {
            ...
            await axios.post(
              `${this.baseUrl}/beli/retur_rfid_delete`,
              { rfid },
              this.tokenHeaders
            );
    ...
    async submitRfid() {
      ...
      try {
        // ================= SIMPAN KE API RFID =================
        await axios.post(
          `${this.baseUrl}/beli/retur_rfid_add`,
          {
            header_id: this.form.id,
    ...
    async loadRfidFromApi() {
      ...
      try {
        const res = await axios.post(
          `${this.baseUrl}/beli/retur_rfid_show`,
          {
            header_id: this.form.id,
...
<style scoped src="@/apps/beli/returdetail.css"></style>
