      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">
            <router-link :to="{ name: 'POS' }"> P.O.S. </router-link>
          </li>
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" v-model="form.station_id">
                ...
                <option v-for="g in stationList" :key="g.id" :value="g.id">
                  {{ g.nama }}
                </option>
              </select>
            </div>
            ...
            <div class="mb-3">
              ...
              <select class="form-select" v-model="form.kasir_id">
                <option value="">Pilih Kasir</option>
                <option v-for="g in kasirList" :key="g.id" :value="g.id">
                  {{ g.nama }}
                </option>
              </select>
            </div>
            ...
            <div class="mb-3">
              ...
              <select class="form-select" v-model="form.salesman_id">
                <option value="">Pilih Salesman</option>
                <option v-for="g in salesmanList" :key="g.id" :value="g.id">
                  {{ g.nama }}
                </option>
              </select>
            </div>
...
export default {
  ...
  methods: {
    ...
    async saveItem(item, index) {
      ...
      try {
        const res = await this.$axios.post(
          `${this.baseUrl}/sales/pos_detail_add`,
          {
            id: item.id || null,
    ...
    async deleteItem(id, index) {
      ...
      await this.$axios.post(
        `${this.baseUrl}/sales/pos_detail_delete`,
        { id },
        this.tokenHeaders
      );
    ...
    async initData() {
      ...
      const res = await axios.get(
        `${this.baseUrl}/sales/pos_show/${this.form.id}`,
        this.tokenHeaders
      );
    ...
    async loadStation() {
      const r = await axios.get(`${this.baseUrl}/profile/station_combo`, this.tokenHeaders);
      this.stationList = r.data || [];
    },

    async loadKasir() {
      const r = await axios.get(`${this.baseUrl}/master/karyawan_combo`, this.tokenHeaders);
      this.kasirList = r.data || [];
    },
      ...
      if (!this.rfidTable) {
        this.$nextTick(async () => {
          ...
          $("#tableRfid").on("click", ".btn-del", async (e) => {
            ...
            await axios.post(
              `${this.baseUrl}/sales/pos_rfid_delete`,
              { rfid },
              this.tokenHeaders
            );
    ...
    async submitRfid() {
      ...
      try {
        // ================= SIMPAN KE API RFID =================
        await axios.post(
          `${this.baseUrl}/sales/pos_rfid_add`,
          {
            header_id: this.form.id,
    ...
    async loadRfidFromApi() {
      ...
      try {
        const res = await axios.post(
          `${this.baseUrl}/sales/pos_rfid_show`,
          {
            header_id: this.form.id,
...
<style scoped src="@/apps/sales/posdetail.css"></style>
