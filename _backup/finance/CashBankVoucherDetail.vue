      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">
            <router-link :to="{ name: 'CashBankVoucher' }"> Cash Bank Voucher </router-link>
          </li>
      ...
      <div class="col-lg-4">
        <div class="card shadow-sm sticky-top" style="top: 90px">
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              <label class="form-label">Akun Sumber</label>
              <select class="form-select" v-model="form.akun_sumber_id">
                <option value="">Pilih Akun Sumber</option>
                <option v-for="g in akunsumberList" :key="g.id" :value="g.id">
                  {{ g.nama }}
                </option>
              </select>
            </div>
...
export default {
  name: "Cash Bank VoucherDetailForm",
  ...
  methods: {
    ...
    async submitForm() {
      try {
        await this.$axios.post(
          `${this.baseUrl}/finance/cash_bank_voucher_add`,
          {
            ...this.form,
    ...
    async saveItem(item, index) {
      ...
      try {
        const res = await this.$axios.post(
          `${this.baseUrl}/finance/cash_bank_voucher_detail_add`,
          {
            id: item.id || null,
    ...
    async deleteItem(id, index) {
      ...
      await this.$axios.post(
        `${this.baseUrl}/finance/cash_bank_voucher_detail_delete`,
        { id },
        this.tokenHeaders
      );
    ...
    async initData() {
      ...
      const res = await axios.get(
        `${this.baseUrl}/finance/cash_bank_voucher_show/${this.form.id}`,
        this.tokenHeaders
      );
      ...
      if (!this.rfidTable) {
        this.$nextTick(async () => {
          ...
          $("#tableRfid").on("click", ".btn-del", async (e) => {
            ...
            await axios.post(
              `${this.baseUrl}/finance/cash_bank_voucher_rfid_delete`,
              { rfid },
              this.tokenHeaders
            );
    ...
    async submitRfid() {
      ...
      try {
        // ================= SIMPAN KE API RFID =================
        await axios.post(
          `${this.baseUrl}/finance/cash_bank_voucher_rfid_add`,
          {
            header_id: this.form.id,
    ...
    async loadRfidFromApi() {
      ...
      try {
        const res = await axios.post(
          `${this.baseUrl}/finance/cash_bank_voucher_rfid_show`,
          {
            header_id: this.form.id,
...
<style scoped src="@/apps/finance/cashbankvoucherdetail.css"></style>
