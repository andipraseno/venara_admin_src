    ...
    <div class="mb-3 mt-3">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">Cash Bank Voucher</li>
        </ol>
      </nav>
    </div>
        ...
        <form @submit.prevent="submitForm" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-floating mb-3">
              <select
                class="form-select"
                v-model="form.akun_sumber_id"
                id="formAkun SumberAsal"
                :class="{ 'is-invalid': errors.akun_sumber_id }"
              >
                <option value="">Pilih Akun Sumber</option>
                <option v-for="s in akunsumberList" :key="s.id" :value="s.id">
                  {{ s.nama }}
                </option>
              </select>
              <label for="formAkun SumberAsal" class="form-label">Akun Sumber</label>
              <div class="invalid-feedback" v-if="errors.akun_sumber_id">
                {{ errors.akun_sumber_id[0] }}
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    // load combo group
    async loadAkunSumberList() {
      try {
        const url = `${this.baseUrl}/master/akun_combo`;
        const res = await axios.get(url, this.tokenHeaders);
        this.akunsumberList = res.data || [];
      } catch (e) {
        console.error("loadAkunSumberList", e);
    ...
    // init datatable
    initDataTable() {
      const url = `${this.baseUrl}/finance/cash_bank_voucher`;
    ...
    async openEditModal(id) {
      ...
      const url = `${this.baseUrl}/finance/cash_bank_voucher_show/${id}`;
      try {
        ...
        this.form.akun_sumber_id = data.akun_sumber_id ?? "";
    ...
    async submitForm() {
      ...
      const url = `${this.baseUrl}/finance/cash_bank_voucher_add`;
      ...
      const payload = {
        ...
        akun_sumber_id: this.form.akun_sumber_id,
        memo: this.form.memo,
...
<style scoped src="@/apps/finance/cashbankvoucher.css"></style>
