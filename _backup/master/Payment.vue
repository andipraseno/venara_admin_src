  ...
  methods: {
    ...
    initDataTable() {
      ...
      this.table = $("#tableMain").DataTable({
        ...
        pageLength: 25,
        ajax: {
          url: `${this.baseUrl}/master/payment`,
          type: "GET",
    ...
    async openEditModal(id) {
      ...
      try {
        const res = await axios.get(
          `${this.baseUrl}/master/payment_show/${id}`,
          this.tokenHeaders
        );
        const data = res.data || {};
    ...
    async submitForm() {
      ...
      try {
        await axios.post(`${this.baseUrl}/master/payment_add`, payload, this.tokenHeaders);
...
<style scoped src="@/apps/master/Payment.css"></style>
